class Solution:
    def combinationSum2(self, candidates: List[int], target: int) -> List[List[int]]:
        answer = set()

        def backtrack(index, subset, cur_sum):
            if cur_sum > target:
                return
            if cur_sum == target:
                answer.add(tuple(subset))
                return
            if index == len(candidates):
                return
            
            # Case 1: Use the current candidate
            subset.append(candidates[index])
            backtrack(index + 1, subset, cur_sum + candidates[index])
            subset.pop()

            # Case 2: Skip the current candidate and all identical successors - If we don't have the while loop, it works as qell, but we get TLE.
            next_index = index + 1
            while next_index < len(candidates) and candidates[next_index] == candidates[index]:
                next_index += 1
            backtrack(next_index, subset, cur_sum)

        candidates.sort()
        backtrack(0, [], 0)

        ret = []
        for tup in answer:
            ret.append(list(tup))
        return ret